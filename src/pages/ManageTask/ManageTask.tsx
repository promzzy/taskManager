...
const ManageTask: FC<ManageTaskProps> = ({ isEdit, onClose }) => {
  const { state, allMembers, dispatch, createTask } = useManageTask({
    isEdit,
    onClose,
  });

  const { title, assignee, status, description, priority, dueDate } = state;
            ...
            <Dropdown
              ...
              onChange={({ target }) => {
                const findAssignee = allMembers.find(
                  (assignee: userInfo) => assignee.id === Number(target.value)
                );
                if (findAssignee) {
